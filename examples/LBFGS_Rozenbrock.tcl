...
proc surface3dData {xrange yrange} {
    ...
    foreach t0 [lseq {*}$xrange] {
        ...
        foreach t1 [lseq {*}$yrange] {
            set x $t1
            set z [dget [fRosenbrock [list $x $y] {}] f]
...
foreach genTr $trajectory genF $bestf {
    lappend optData [list {*}[dict get $genTr x] [dict get $genF f]]
    lappend functionTrajectory [list [dict get $genTr iter] [dict get $genF f]]
}
...
puts "Number of iterations: [dget $results niter]"
...
$chart Xaxis -name "Iteration" -minorTick {show "True"} -type "value" -splitLine {show "True"}
$chart Yaxis -name "Rozenbrock function value" -minorTick {show "True"} -min 1e-12 -max 10000 -type "log" -splitLine\
        {show "True"}
